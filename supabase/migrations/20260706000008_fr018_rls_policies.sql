...
DROP POLICY IF EXISTS "Operators can view own profile" ON public.profiles;
CREATE POLICY "Operators can view own profile" 
ON public.profiles FOR SELECT TO authenticated
USING (auth.uid() = id);

-- Operators can update their own display_name
DROP POLICY IF EXISTS "Operators can update own profile" ON public.profiles;
...
DROP POLICY IF EXISTS "Operators can view own videos" ON public.videos;
CREATE POLICY "Operators can view own videos" 
ON public.videos FOR SELECT TO authenticated
USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Operators can insert own videos" ON public.videos;
CREATE POLICY "Operators can insert own videos" 
ON public.videos FOR INSERT TO authenticated
WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "Operators can update own videos" ON public.videos;
...
DROP POLICY IF EXISTS "Service role can do everything on videos" ON public.videos;
...
DROP POLICY IF EXISTS "Operators can view own violations" ON public.violations;
CREATE POLICY "Operators can view own violations" 
ON public.violations FOR SELECT TO authenticated
USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Operators can insert own violations" ON public.violations;
CREATE POLICY "Operators can insert own violations" 
ON public.violations FOR INSERT TO authenticated
WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "Operators can update own violations" ON public.violations;
...
-- Service role bypasses
DROP POLICY IF EXISTS "Service role can do everything on violations" ON public.violations;
